...
// Given a log entry captured by LogTarget (above) parse out the various fields
// and return them as a map.
//
// The fields parsed are "time", "level", "msg", "error", "function",
...
// generated by a function named funcName and a "msg=" that matches the regular
// expression logEntryRE, within the most recent maxEntries lines of the log.
//
...
//
// Example: logEntryRE can be very simple, but here is a complicated one from
// trackedlock/api_test.go that matches the following message (where "\n" in the
// message represents embededded newlines).
//
...
func ParseLogForFunc(logcopy LogTarget, funcName string, logEntryRE *regexp.Regexp, maxEntries int) (fields map[string]string, entryIdx int, err error) {
	return parseLogForFunc(logcopy, funcName, logEntryRE, maxEntries)
}
